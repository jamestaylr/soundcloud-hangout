    ...
    if (key == 'state') {
        ...
        widget
            .getPosition(function(data) {
                if (Math.abs(parseInt(data) - parseInt(state.currentPosition)) >
                    1000) {
...
function loadTrack(track) {
    current = track;
    var query = track.uri + "?client_id=" + client_id +
        "&amp;auto_play=true";
    widget.load(query);
}
...
function queueTrack(track) {
    ...
    var artwork = track.artwork_url;
     ...
     var result = '<tr id=\"' + track.id + '\"><td>';
    
    
    if (artwork != undefined)
    {
    	result += '<img src=\"' + artwork + '\" \\>';
    }
    
    result += '</td><td class=\"title\">' + track.title +
        ' by ' + '<a href=\"' + track.user.uri + '\">' + track.user.username +
        '</a>' + '</td><td class=\"time\">' + minutes + ':' + (((seconds %
            60) / 100).toFixed(2).toString()).replace('0.', '') +
        '</td><td class=\"likes\">' + track.favoritings_count.toString().replace(
            /\B(?=(\d{3})+(?!\d))/g, ",") + '</td><td class=\"plays\">' +
...
function createEmbededSound(event) {
    ...
    SC.get('/tracks', {
        ...
        for (var i = 0;
            (i < tracks.length) && (i < 5); i++) {
            searchResults.push(tracks[i]);
            r.innerHTML += '<li class=\"result\" value=\"' + tracks[
                i].id + '\">' + tracks[i].title + '</li>';
        }
    });
